...
paths:
  /inbound:
    post:
      summary: MaloIdent Anfrage auslösen
      deprecated: false
      description: >-
        `START_MALOIDENT`


        Triggert den Versand einer MaloIdent Anfrage via API-Webdienst an den
        Netzbetreiber durch die MaloIdent APP. Die Anfrage wird identifiziert
        durch den Eventnamen START_MALOIDENT. Zusätzlich ist eine eindeutige ID
        prozessId aus dem Backend mit zu übergeben, mit der die spätere Antwort
        vom Netzbetreiber wieder an das Backend übergeben werden kann.
      ...
      requestBody:
        content:
          application/json:
            schema:
              $ref: '#/components/schemas/%5BLF%5D%20START_MALOIDENT'
            example:
              stammdaten:
                MARKTLOKATION:
                  - boTyp: MARKTLOKATION
      ...
      x-run-in-apidog: https://app.apidog.com/web/project/816353/apis/api-15144286-run
components:
  schemas:
    '[LF] START_MALOIDENT':
      type: object
